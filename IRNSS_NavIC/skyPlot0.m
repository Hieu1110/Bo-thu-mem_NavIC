...
for i = 1:num_satellites
    if any(xx(i, :) ~= 0) && any(yy(i, :) ~= 0)
        if strcmp(line_style, 'auto')
            %--- Plot with "default" line style -----------------------------------
            hpol(i) = plot(hAxis, xx(i, :)', yy(i, :)', '.-', 'Color', colors(i, :));
        else
            %--- Plot with user specified line style ------------------------------
            hpol(i) = plot(hAxis, xx(i, :)', yy(i, :)', line_style, 'Color', colors(i, :));
        end

        %--- Mark the last position of the satellite ------------------------------
        plot(hAxis, xx(i, end)', yy(i, end)', 'o', 'MarkerSize', 7, 'Color', colors(i, :));

        %--- Place satellite PRN numbers at the latest position -------------------
        if(prn(i) ~= 0)
            % Adjust text position slightly to avoid overlapping with edge
            text(xx(i, end), yy(i, end), ['  ', int2str(prn(i))], 'color', colors(i, :));
            % Add to legend only if PRN is non-zero and plotted

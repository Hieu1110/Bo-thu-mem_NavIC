...
%% Plot all figures =======================================================
        ...
        set(channelNr +300, 'Name', ['Channel ', num2str(channelNr), ...
            ...
            num2str(trackResults(channelNr).PRN), ...
            ') CNo']);
        plot(trackResults(channelNr).CNo.VSMValue);hold on
        plot(trackResults(channelNr).CNo.VSMValue,'mo');hold off
        title('CNo Estimation (computed only every 400msec)')
        ylabel('dB-Hz')
        xlabel('400msec epoch computation')
